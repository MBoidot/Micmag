...
def extract_grain_image(grain_polygon, image, image_pred=None, target_size=224, pad=10):
    ...
    grain_crop = np.array(
        Image.fromarray(grain_crop).rotate(
            -np.rad2deg(orientation), fillcolor=0, expand=True
        )
    )
    mask_rotated = (
        np.array(
            Image.fromarray((mask_squared * 255).astype(np.uint8)).rotate(
                -np.rad2deg(orientation), fillcolor=0, expand=True
            )
        )
        / 255.0
    ...
    grain_image = tf.image.resize(
        grain_crop, (target_size, target_size), antialias=False
    )
    grain_image = np.array(grain_image).astype("uint8")

    grain_mask = tf.image.resize(
        mask_rotated[..., None], (target_size, target_size), antialias=True
    )
    ...
    grain_pred = None
    if pred_crop_masked is not None:
        pred_crop_rotated = np.array(
            Image.fromarray(pred_crop_masked).rotate(
                -np.rad2deg(orientation), fillcolor=0, expand=True
            )
        )
        pred_crop_rotated = make_square(pred_crop_rotated)
        grain_pred = tf.image.resize(
            pred_crop_rotated[..., None], (target_size, target_size), antialias=False
        )
...
def extract_vgg16_features(
    ...
    if isinstance(layer_name, int) or (
        isinstance(layer_name, str) and layer_name.startswith("-")
    ):
    ...
    if grain_batch.shape[1] != 224 or grain_batch.shape[2] != 224:
        ...
        logger.info(
            f"Resizing grain images from {grain_batch.shape[1]}x{grain_batch.shape[2]} to 224x224"
        )
    ...
    features = feature_model.predict(
        grain_batch, batch_size=batch_size, verbose=verbose
    )
...
def cluster_grains(
    features, n_clusters=10, method="kmeans", n_components=25, random_state=42, **kwargs
):
    """
    ...
    **kwargs : dict
    ...
    if n_components is not None:
        ...
        print(
            f"PCA: {features.shape[1]} features → {n_components} components "
...
def create_grain_panel_cluster(cluster_number, labels, all_images):
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            cl_panel[i * 224 : (i + 1) * 224, j * 224 : (j + 1) * 224, :] = all_images[
                rand_inds[i * n_cols + j]
            ]
...
def create_clustered_grain_montage(
    cluster_labels,
    grain_images,
    grid_cols=20,
    grain_size=224,
    ...
    for i, grain_idx in enumerate(ordered_grain_indices):
        ...
        montage[y_pos : y_pos + grain_size, x_pos : x_pos + grain_size] = grain_img
...
class ClusterMontageSelector:
                ...
                if grain_idx in self.removed_grain_indices:
                    ...
                    for i in range(self.grain_size):
                        for t in range(-line_thickness // 2, line_thickness // 2 + 1):
                            ...
                            x_main = x_pos + i + t
                            if (
                                0 <= y_main < display_img.shape[0]
                                and 0 <= x_main < display_img.shape[1]
                            ):
                            ...
                            if (
                                0 <= y_anti < display_img.shape[0]
                                and 0 <= x_anti < display_img.shape[1]
                            ):
                    ...
                    overlay = display_img[
                        y_pos : y_pos + self.grain_size, x_pos : x_pos + self.grain_size
                    ].copy()
                    overlay = (overlay * 0.5 + np.array([255, 0, 0]) * 0.5).astype(
                        np.uint8
                    )
                    display_img[
                        y_pos : y_pos + self.grain_size, x_pos : x_pos + self.grain_size
    ...
    def get_filtered_images(self):
        ...
        return [
            img
            for i, img in enumerate(self.grain_images)
            if i not in self.removed_grain_indices
        ]
    ...
    def get_filtered_indices(self):
        ...
        return np.array(
            [
                i
                for i in range(len(self.all_grains))
                if i not in self.removed_grain_indices
            ]
        )
...
class ClusterMontageLabeler:
    ...
    def __init__(
        ...
        if label_colors is None:
            ...
            self.label_colors = {
                name: color for name, color in zip(self.label_names, label_colors)
            }
        ...
        self.montage_artist = self.ax.imshow(
            self.montage.astype("uint8"), animated=False
        )
        self.ax.axis("off")
    ...
    def _draw_grain_border(self, grain_idx, label):
        ...
        # Find position
        cluster_id = self.grain_to_cluster[grain_idx]
        pos_idx = np.where(self.cluster_info[cluster_id]["grain_indices"] == grain_idx)[
            0
        ]
        if len(pos_idx) == 0:
            return []
        ...
        patches = [
            ...
            Rectangle(
                (x_pos, y_pos + self.grain_size - thickness),
                self.grain_size,
                thickness,
                facecolor=color,
                edgecolor="none",
            ...
            Rectangle(
                (x_pos + self.grain_size - thickness, y_pos),
                thickness,
                self.grain_size,
                facecolor=color,
                edgecolor="none",
    ...
    def onclick(self, event):
        ...
        print(
            f"Labeled {n_labeled}/{len(self.all_grains)} grains | Active: [{self.active_label}]"
        )
    ...
    def get_unlabeled_indices(self):
        ...
        return np.array(
            [i for i in range(len(self.all_grains)) if i not in self.grain_labels]
        )
    ...
    def export_labels(self, filepath):
        ...
        data = {
            ...
            "cluster_id": [
                self.grain_to_cluster[idx] for idx in self.grain_labels.keys()
            ],
        }
    ...
    def save_labeled_images(self, output_dir):
        ...
        for grain_idx, label in tqdm(
            self.grain_labels.items(), desc="Saving labeled images"
        ):
            # Get the grain image
