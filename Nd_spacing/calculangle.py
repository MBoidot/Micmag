...
Returns:
    ...
    distri  : per-row mean of (angle) computed on bright pixels
...
distri is now computed as the mean of the actual angles (not 90 - |angle|) over bright pixels.
uning tips
    ...
    for i in range(hauteur):
        ...
        if np.count_nonzero(row_mask) > 0:
            ...
            if row_angles.size > 0:
                distri[i] = np.mean(np.abs(row_angles))
